...
export default function NewTweeb({ add }: { add: () => void }) {
  ...
  return (
    <div className="flex flex-col justify-between items-start gap-2 w-full">
      <input
        className="w-full border px-4 py-2 outline-none text-slate-200 bg-slate-900 border-none"
        type="text"
        name="title"
        id="title"
        value={tweeb.title}
      ...
      {uploads && (
        <div className="flex gap-3 px-4">
          {uploads.map((file, index) => (
            <img key={index} className="w-12 h-12" src={file} />
          ))}
          <button
            onClick={() => setUploads(undefined)}
